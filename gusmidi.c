...
//FIXME      gf1_unload_patch( &Patch[ prog ] );
...
static int GUSMIDI_LoadPatch
   ...
   ret = !OK; //FIXME gf1_get_patch_info( text, &patchi );
   if ( ret != OK )
      {
      GUS_AuxError = ret;
...
void GUSMIDI_ProgramChange
   ...
   if ( PatchLoaded[ prog ] )
      {
//FIXME      gf1_midi_change_program( &Patch[ prog ], channel );
      }
   else
      {
//FIXME      gf1_midi_change_program( NULL, channel );
      }
   }
...
void GUSMIDI_NoteOn
   ...
   if ( chan == 9 )
      ...
      if ( PatchLoaded[ prog ] )
         {
//FIXME         gf1_midi_note_on( &Patch[ note + 128 ], 1, note, velocity, 9 );
         }
      }
   else
      {
//FIXME      gf1_midi_note_on( 0L, 1, note, velocity, chan );
      }
   }
...
void GUSMIDI_NoteOff
   ...
   {
//FIXME   gf1_midi_note_off( note, chan );
   }
#pragma warn .par
...
void GUSMIDI_ControlChange
   ...
   {
//FIXME   gf1_midi_parameter( channel, number, value );
   }
...
void GUSMIDI_PitchBend
   ...
   {
//FIXME   gf1_midi_pitch_bend( channel, lsb, msb );
   }
...
void GUSMIDI_SetVolume
   ...
   // range = 0 to 127
//FIXME   gf1_midi_synth_volume( 0, volume >> 1 );
   }
