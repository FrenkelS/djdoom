...
static boolean WritePressKey(byte channel, byte key, byte velocity, FILE *midioutput)
{
...
static boolean WriteReleaseKey(byte channel, byte key, FILE *midioutput)
{
...
static boolean WritePitchWheel(byte channel, short wheel, FILE *midioutput)
{
...
static boolean WriteChangePatch(byte channel, byte patch, FILE *midioutput)
{
...
static boolean WriteChangeController_Valued(byte channel, byte control, byte value, FILE *midioutput)
{
...
static boolean WriteChangeController_Valueless(byte channel, byte control, FILE *midioutput)
{
...
{
	return fread(&header->id,                sizeof(byte),    4, file) == 4
        && fread(&header->scorelength,       sizeof(int16_t), 1, file) == 1
        && fread(&header->scorestart,        sizeof(int16_t), 1, file) == 1
        && fread(&header->primarychannels,   sizeof(int16_t), 1, file) == 1
        && fread(&header->secondarychannels, sizeof(int16_t), 1, file) == 1
        && fread(&header->instrumentcount,   sizeof(int16_t), 1, file) == 1;
}
		...
		while (!hitscoreend)
		{
			...
			switch (event)
			{
				...
				case mus_systemevent:
                    ...
                    {
						// All notes off
						if (WriteChangeController_Valueless(channel, 0x78, midioutput))
							return true;
						...
						// Reset pan
						if (WriteChangeController_Valued(channel, 0x0a, 64, midioutput))
							return true;
